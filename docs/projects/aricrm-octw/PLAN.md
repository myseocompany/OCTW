...
- [x] Create `BASELINE.md` with OCTW/upstream SHA, clean AriCRM audit SHA, OpenClaw version/digest, tool versions, commands, results, limitations, date, and reviewer. (`SPEC.md` §14)
...
- [x] Record which findings remain after source commits `6c94917`, `450f302`, and `299c33b`; do not infer closure from commit titles. (`SPEC.md` §14; `STATUS.md`)
- [x] Approve `GOAL.md` and `SPEC.md`, and resolve formerly Proposed decisions D-010 through D-013. (`SPEC.md` status; `DECISIONS.md`)
- [x] Obtain privacy/legal and operations approval for D-013 pilot values. (`SPEC.md` §3 and §10.1)
- [x] Define CI, branch protection, releases, dependency updates, and the environment matrix. (`SPEC.md` §14; `CI_PLAN.md`)
...
- [ ] Dispatch OCTW jobs only after the AriCRM transaction commits, using an outbox or equivalent durable handoff; the current database queue uses `after_commit=false`. (`SPEC.md` §5 and §6)
...
- [ ] Add a dedicated scoped service identity for OCTW; do not reuse human sessions or existing public webhook routes. (`SPEC.md` §4)
- [ ] Add an explicit platform-admin authorization policy/gate for OCTW operations; AriCRM's current admin routes use only `auth` middleware. (`SPEC.md` §4)
